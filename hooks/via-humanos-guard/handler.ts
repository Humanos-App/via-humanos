...
async function _checkMandateViaApi(
  ...
  try {
    ...
    console.error(`[via-humanos-guard] API check failed: ${message}`);
...
const handler = async (event: HookEvent): Promise<void> => {
  ...
  console.log(`[via-humanos-guard] Checking mandate for tool: ${toolName}`);
  ...
  if (!result.valid) {
    ...
    event.messages.push(
      `Action blocked by VIA Mandate Guard. ${reason}. ` +
        `Use the via-humanos skill to create an approval request before proceeding. ` +
        `Example: "I need approval from manager@company.com to ${scope}"`
    );
    console.log(`[via-humanos-guard] BLOCKED: ${toolName} — ${reason}`);
    ...
    console.log(
      `[via-humanos-guard] ALLOWED: ${toolName} — mandate ${result.mandate?.id} valid until ${result.mandate?.validUntil}`
    );
  }
};
